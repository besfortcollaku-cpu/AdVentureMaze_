...
export function mountUI(app) {
  app.innerHTML = `
    ...
    <!-- 鉁� LOGIN GATE (blocks game until Pi login) -->
    ...
    <div class="settingsOverlay" id="settingsOverlay" aria-hidden="true">
      <div class="settingsCard">
        <div class="settingsHeader">
          ...
          <button class="settingsClose" id="settingsCloseBtn" aria-label="Close">鉁�</button>
        </div>
...
<div class="welcomeOverlay" id="welcomeOverlay" aria-hidden="false">
  ...
  // 鉁� Inject UI styles (Login Gate + Settings + Win overlay + login wrap)
  ...
  // 鉁� FIXED (single correct implementation)
  function showLevelSelect({ totalLevels, isCompleted, currentLevel }) {
    ...
    for (let i = 1; i <= totalLevels; i++) {
      ...
      btn.textContent = completed ? `鉁� Level ${i}` : `馃敀 ${i}`;
  ...
  // 鉁� first user gesture (for WebAudio unlock on mobile)
...
// 馃殌 AUTO LOGIN (only after handler is registered)
setTimeout(() => {
  ...
  function showLoginGate() {
      // 馃殌 auto trigger login once visible
setTimeout(() => {
  loginGateClickHandler?.();
}, 200);
    if (!loginGate) return;
  ...
  loginGateClickHandler = async () => {
    try {
      ...
      // 鉁� logged in
      if (loginBtnText) {
        loginBtnText.textContent = "Logged in 鉁�";
      }

      // 鉁� fade out welcome screen
  ...
  return {
    ...
    // 鉁� login gate methods for ensurePiLogin()
    showLoginGate,
    hideLoginGate,
    showLoginError,
    onLoginClick,
    ...
    // Win popup API (鉁� kept only once)
    showWinPopup,
    hideWinPopup,
    onWinNext(fn) {
      winNextHandler = fn;
