...
export function mountUI(app) {
  app.innerHTML =
      ...
      '<div class="topbar">' +
        '<div class="topRow">' +
          ...
          '<div class="levelWrap">' +
            ...
            '<div class="levelText" id="levelLabel">LEVEL 1</div>' +
          "</div>" +
    ...
    // ✅ Desktop / Not allowed overlay (used by piDetect)
    '<div class="desktopBlock" id="desktopBlock" style="display:none;">' +
      '<div class="desktopCard">' +
        "<h2>Mobile game</h2>" +
        "<p>Please open inside Pi Browser on mobile. (Dev: add ?dev=true)</p>" +
      "</div>" +
    "</div>" +

    // ✅ Level complete overlay
    '<div id="levelCompleteOverlay" style="display:none; position:fixed; inset:0; z-index:9999; background:rgba(0,0,0,0.82);">' +
      '<div style="height:100%; display:flex; align-items:center; justify-content:center; padding:18px;">' +
        '<div style="width:min(520px, 92vw); border-radius:20px; border:1px solid rgba(255,255,255,0.12); background:rgba(18,28,60,0.85); padding:18px;">' +
          '<div style="font-size:22px; font-weight:900; color:#fff; margin-bottom:6px;" id="levelCompleteTitle">Level Complete!</div>' +
          '<div style="color:rgba(234,243,255,0.85); font-weight:700; margin-bottom:14px;" id="levelCompleteText">Congrats 🎉</div>' +
          '<div style="display:flex; gap:10px; flex-wrap:wrap;">' +
            '<button id="nextLevelBtn" style="flex:1; min-width:180px; height:46px; border-radius:14px; border:1px solid rgba(255,255,255,.18); background: rgba(37,215,255,.22); color:#fff; font-weight:900; cursor:pointer;">Next Level</button>' +
            '<button id="watchAdBtn" style="flex:1; min-width:180px; height:46px; border-radius:14px; border:1px solid rgba(255,255,255,.18); background: rgba(255,210,120,.18); color:#fff; font-weight:900; cursor:pointer;">Watch an Ad (+10)</button>' +
          "</div>" +
        "</div>" +
      "</div>" +
    "</div>";

  // ✅ small CSS injection (keeps you from editing style.css)
  ...
  return {
    ...
    overlayTitle: document.getElementById("levelCompleteTitle"),
    overlayText: document.getElementById("levelCompleteText"),
